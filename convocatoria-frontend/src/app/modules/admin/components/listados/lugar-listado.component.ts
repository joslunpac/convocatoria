...
import { LugarDetalleComponent } from '..';
import { ModoEnum } from '../../../../core/enumerates';
import { Lugar } from '../../../../core/interfaces';
import { LugarService, TokenService } from '../../../../core/services';
...
const ENTIDAD = AC.LUGAR;
const ENTIDAD_PLURAL = AC.LUGARES;
 ...
 */
@Component({
  selector: 'app-lugar-listado',
  templateUrl: './lugar-listado.component.html'
})
export class LugarListadoComponent implements OnInit {
  ...
  obtenerListadoCompleto(): void {
    ...
    this.lugService.findAll(ordenacion).subscribe((data: Lugar[]) => {
  ...
  configurarDataSource(): void {
    ...
    this.dataSource.filterPredicate = (data: Lugar, filter: string): boolean => {
      let cadenaAFiltrar = '';

      // Componemos la cadena a filtrar en función de las columnas deseadas
      cadenaAFiltrar += data.nombre + ' ';
      cadenaAFiltrar += data.direccion + ' ';
   ...
   */
  irADetalle(modo: string = ModoEnum.CREAR, objeto?: Lugar): void {
    // Abrimos el diálogo
    const dialogoDetalle = this.dialog.open(LugarDetalleComponent, {
      //disableClose: true,
      minWidth: '40%',
      data: {
        modo: modo,
  ...
  eliminar(objeto: Lugar): void {
    ...
    dialogoEliminar.afterClosed().subscribe((confirmado: Boolean) => {
      if (confirmado) {
        ...
        this.lugService.delete(objeto.id).subscribe(() => {
          // Construimos la notificación
