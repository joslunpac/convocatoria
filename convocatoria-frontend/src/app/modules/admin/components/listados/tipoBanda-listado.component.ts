...
import { TipoBandaDetalleComponent } from '..';
import { ModoEnum } from '../../../../core/enumerates';
import { TipoBanda } from '../../../../core/interfaces';
import { TipoBandaService, TokenService } from '../../../../core/services';
...
const ENTIDAD = AC.TIPO_BANDA;
const ENTIDAD_PLURAL = AC.TIPOS_BANDA;

/**
 * Listado de tipos de banda
 *
 ...
 */
@Component({
  selector: 'app-tipoBanda-listado',
  templateUrl: './tipoBanda-listado.component.html',
  styleUrls: ['./tipoBanda-listado.component.scss'],
})
export class TipoBandaListadoComponent implements OnInit {
  ...
  obtenerListadoCompleto(): void {
    ...
    this.tbaService.findAll(ordenacion).subscribe((data: TipoBanda[]) => {
  ...
  configurarDataSource(): void {
    ...
    this.dataSource.filterPredicate = (data: TipoBanda, filter: string): boolean => {
      let cadenaAFiltrar = '';
   ...
   */
  irADetalle(modo: string = ModoEnum.CREAR, objeto?: TipoBanda): void {
    // Abrimos el diálogo
    const dialogoDetalle = this.dialog.open(TipoBandaDetalleComponent, {
      //disableClose: true,
      minWidth: '40%',
      data: {
        modo: modo,
   ...
   */
  eliminar(objeto: TipoBanda): void {
    // Abrimos el diálogo
    ...
    dialogoEliminar.afterClosed().subscribe((confirmado: Boolean) => {
      if (confirmado) {
        ...
        this.tbaService.delete(objeto.id).subscribe(() => {
          // Construimos la notificación
