...
import { TipoActoCultoDetalleComponent } from '..';
import { ModoEnum } from '../../../../core/enumerates';
import { TipoActoCulto } from '../../../../core/interfaces';
import { TipoActoCultoService, TokenService } from '../../../../core/services';
...
const ENTIDAD = AC.TIPO_ACTO_CULTO;
const ENTIDAD_PLURAL = AC.TIPOS_ACTO_CULTO;

/**
 * Listado de tipos de acto o culto
 *
 ...
 */
@Component({
  selector: 'app-tipoActoCulto-listado',
  templateUrl: './tipoActoCulto-listado.component.html',
  styleUrls: ['./tipoActoCulto-listado.component.scss'],
})
export class TipoActoCultoListadoComponent implements OnInit {
  ...
  obtenerListadoCompleto(): void {
    ...
    this.tacService.findAll(ordenacion).subscribe((data: TipoActoCulto[]) => {
  ...
  configurarDataSource(): void {
    ...
    this.dataSource.filterPredicate = (data: TipoActoCulto, filter: string): boolean => {
      let cadenaAFiltrar = '';
   ...
   */
  irADetalle(modo: string = ModoEnum.CREAR, objeto?: TipoActoCulto): void {
    // Abrimos el diálogo
    const dialogoDetalle = this.dialog.open(TipoActoCultoDetalleComponent, {
      //disableClose: true,
      minWidth: '40%',
      data: {
        modo: modo,
   ...
   */
  eliminar(objeto: TipoActoCulto): void {
    // Abrimos el diálogo
    ...
    dialogoEliminar.afterClosed().subscribe((confirmado: Boolean) => {
      if (confirmado) {
        ...
        this.tacService.delete(objeto.id).subscribe(() => {
          // Construimos la notificación
