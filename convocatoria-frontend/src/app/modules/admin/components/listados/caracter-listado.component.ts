...
import { CaracterDetalleComponent } from '..';
import { ModoEnum } from '../../../../core/enumerates';
import { Caracter } from '../../../../core/interfaces';
import { CaracterService, TokenService } from '../../../../core/services';
...
const ENTIDAD = AC.CARACTER;
const ENTIDAD_PLURAL = AC.CARACTERES;

/**
 * Listado de carácteres
 *
 ...
 */
@Component({
  selector: 'app-caracter-listado',
  templateUrl: './caracter-listado.component.html',
  styleUrls: ['./caracter-listado.component.scss'],
})
export class CaracterListadoComponent implements OnInit {
  ...
  obtenerListadoCompleto(): void {
    ...
    this.carService.findAll(ordenacion).subscribe((data: Caracter[]) => {
  ...
  configurarDataSource(): void {
    ...
    this.dataSource.filterPredicate = (data: Caracter, filter: string): boolean => {
      let cadenaAFiltrar = '';
   ...
   */
  irADetalle(modo: string = ModoEnum.CREAR, objeto?: Caracter): void {
    // Abrimos el diálogo
    const dialogoDetalle = this.dialog.open(CaracterDetalleComponent, {
      //disableClose: true,
      minWidth: '40%',
      data: {
        modo: modo,
  ...
  eliminar(objeto: Caracter): void {
    ...
    dialogoEliminar.afterClosed().subscribe((confirmado: Boolean) => {
      if (confirmado) {
        ...
        this.carService.delete(objeto.id).subscribe(() => {
          // Construimos la notificación
